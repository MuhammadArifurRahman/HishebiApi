...
class DenaAPI {
    ...
    public function getById($id) {
        ...
        if (!$entry) {
            Response::notFound("Dena entry not found");
        }
    ...
    public function create() {
        ...
        $stmt = $this->db->getConnection()->prepare("
            INSERT INTO dena (user_id, name, address, mobile, reason, amount, due_date) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        ...
        ], "Dena entry created successfully");
    }
    ...
    public function update($id) {
        ...
        $sql = "UPDATE dena SET " . implode(", ", $fields) . " WHERE id = ?";
    ...
    public function delete($id) {
        $stmt = $this->db->getConnection()->prepare("DELETE FROM dena WHERE id = ?");
        $stmt->execute([$id]);
        ...
        Response::success(null, "Dena entry deleted successfully");
    }

    private function getTotal() {
        $stmt = $this->db->getConnection()->query("
            SELECT COALESCE(SUM(amount), 0) as total FROM dena
        ");
        return (float)$stmt->fetch()['total'];
    }
}

if (basename($_SERVER['SCRIPT_FILENAME']) === 'dena.php') {
    $api = new DenaAPI();
